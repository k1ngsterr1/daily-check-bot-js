  ...
  */
  ...
  private isReminderRequest(text: string): boolean {
    ...
    const naturalTimePatterns = [
      ...
      // Паттерны с относительным временем
      /через\s*(\d+)\s*(минут|час).*(сделать|выполнить|купить|позвонить|написать|отправить|подготовить|организовать|запланировать|встретить|пойти|поехать)/i, // "через час позвонить"
      /(сделать|выполнить|купить|позвонить|написать|отправить|подготовить|организовать|запланировать|встретить|пойти|поехать).*через\s*(\d+)\s*(минут|час)/i, // "позвонить через час"
    ];
  ...
  private isTaskRequest(text: string): boolean {
    ...
    const taskPatterns = [
      ...
      /(завтра|послезавтра|сегодня|в понедельник|во вторник|в среду|в четверг|в пятницу|в субботу|в воскресенье)\s+(сделать|выполнить|купить|позвонить|написать|отправить|подготовить|организовать|запланировать)/i, // "завтра сделать", "в понедельник купить" БЕЗ времени
    ...
    // Строгие исключения для сообщений с конкретным временем (они должны быть напоминаниями)
    const timeExclusions = [
      /в\s*(\d{1,2}):(\d{2})/i, // "в 15:30" - точное время
      /в\s*(\d{1,2})\s*час/i, // "в 3 часа"
      /через\s*(\d+)\s*(минут|час)/i, // "через 2 часа"
      /(утром|днем|вечером|ночью).*в\s*\d/i, // "вечером в ..."
      /завтра\s+в\s+\d/i, // "завтра в 15:30"
      /сегодня\s+в\s+\d/i, // "сегодня в 15:30"
    ];

    // Исключаем триггеры напоминаний
    const reminderTriggers = [/напомни|напомню|напоминание|remind/i];

    // Проверяем исключения
    const hasTimeExclusions = timeExclusions.some((pattern) =>
      pattern.test(text),
    );
    ...
    if (hasTimeExclusions || hasReminderTriggers) {
      return false;
    }
